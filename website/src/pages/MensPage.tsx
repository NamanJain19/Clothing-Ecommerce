...
import { MenHeroBanner } from '../components/men/MenHeroBanner';
...
import { productService, toUICategoryProduct, toUIProduct } from '../services/productService';
import { Product } from '../types';

export const MensPage: React.FC = () => {
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [products, setProducts] = useState(menProductsData);
  const [totalProducts, setTotalProducts] = useState(menProductsData.length);
    ...
    const fetchMenProducts = async () => {
      ...
      try {
        ...
        if (res.products && res.products.length > 0 && isMounted) {
          ...
          setProducts(menProductsData);
          setTotalProducts(menProductsData.length);
        }
      } catch (err) {
        console.warn('Failed to load men products from API, using fallback:', err);
        if (isMounted) {
          setProducts(menProductsData);
          setTotalProducts(menProductsData.length);
        }
      ...
      <main>
        ...
        <section className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop mt-12">
          {isLoading ? (
            ...
            <div className="py-24 text-center space-y-3">
              ...
              <p className="font-body-md text-secondary text-sm">
                No matching sartorial items are currently available.
              </p>
            </div>
          ) : (
          ...
          <div className="mt-section-gap flex flex-col items-center">
            ...
            <button className="font-label-caps text-label-caps px-12 py-4 border border-primary hover:bg-primary hover:text-white transition-all duration-300 uppercase cursor-pointer">
              LOAD MORE
            </button>
          </div>
        </section>
