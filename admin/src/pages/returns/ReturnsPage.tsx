...
import { RotateCcw, CheckCircle, XCircle, Eye, Search, AlertCircle, Loader2, PackageCheck } from 'lucide-react';
...
export const ReturnsPage: React.FC = () => {
  ...
  const fetchReturns = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await adminService.getReturns({ limit: 100 });
      const rawList = res?.data || [];
      const mapped: ReturnRequest[] = rawList.map((r: any) => ({
        id: r._id || r.id,
        orderNumber: r.order?.orderNumber || (typeof r.order === 'string' ? r.order : 'ORD-LIV'),
        ...
        reason: r.reason || r.adminNotes || 'Client requested return / alteration',
        createdAt: r.createdAt,
      }));
      setReturnsList(mapped);
    } catch (err: any) {
      console.error('Failed to fetch returns:', err);
      setError(err.message || 'Unable to load returns data. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      if (status === 'Authorized') {
        ...
        await adminService.rejectReturn(id, 'Admin rejection');
      } else {
        await adminService.updateReturnStatus(id, status);
      }
      ...
      alert(err.message || 'Failed to update return status in database.');
    }
  };
      ...
      <div className="p-4 sm:p-space-xl max-w-[1440px] mx-auto w-full space-y-space-lg">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white border border-outline-variant rounded-xl">
            ...
            <p className="font-body-md text-sm text-on-surface-variant">Loading client return requests from database...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-16 bg-red-50/50 border border-red-200 rounded-xl text-center px-4">
            ...
            <AdminButton variant="outline" className="mt-4" onClick={fetchReturns}>
              Retry
            </AdminButton>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white border border-outline-variant rounded-xl text-center px-4">
            <PackageCheck className="w-10 h-10 text-on-surface-variant/40 mb-3" />
            <h3 className="font-display text-base font-bold text-primary">No return requests found</h3>
            <p className="font-body-md text-sm text-on-surface-variant mt-1 max-w-sm">
              All deliveries verified. No active returns, exchange requests, or tailoring alteration tickets.
            </p>
          </div>
        ) : (
