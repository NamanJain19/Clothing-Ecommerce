...
import { AccessoriesHeroBanner } from '../components/accessories/AccessoriesHeroBanner';
...
export const AccessoriesPage: React.FC = () => {
  ...
  const [products, setProducts] = useState(accessoriesProductsData);
  const [totalProducts, setTotalProducts] = useState(accessoriesProductsData.length);
    ...
    const fetchAccessories = async () => {
      ...
      try {
        ...
        if (res.products && res.products.length > 0 && isMounted) {
          ...
          setProducts(accessoriesProductsData);
          setTotalProducts(accessoriesProductsData.length);
        }
      } catch (err) {
        console.warn('Failed to load accessories from API, using fallback:', err);
        if (isMounted) {
          setProducts(accessoriesProductsData);
          setTotalProducts(accessoriesProductsData.length);
        }
  ...
  return (
    <div className="min-h-screen bg-background text-primary font-body-md antialiased selection:bg-primary selection:text-on-primary">
      ...
      <main>
        ...
        {/* Standardized Accessories Product Grid */}
          ...
          {isLoading ? (
            ...
            <div className="py-24 text-center space-y-3">
              ...
              <p className="font-body-md text-secondary text-sm">
                No matching accoutrements are currently available.
              </p>
            </div>
          ) : (
