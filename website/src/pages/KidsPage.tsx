...
import { KidsHeroBanner } from '../components/kids/KidsHeroBanner';
...
export const KidsPage: React.FC = () => {
  ...
  const [products, setProducts] = useState(kidsProductsData);
  const [totalProducts, setTotalProducts] = useState(kidsProductsData.length);
    ...
    const fetchKidsProducts = async () => {
      ...
      try {
        ...
        if (res.products && res.products.length > 0 && isMounted) {
          ...
          setProducts(kidsProductsData);
          setTotalProducts(kidsProductsData.length);
        }
      } catch (err) {
        console.warn('Failed to load kids products from API, using fallback:', err);
        if (isMounted) {
          setProducts(kidsProductsData);
          setTotalProducts(kidsProductsData.length);
        }
  ...
  return (
    <div className="min-h-screen bg-background text-primary font-body-md antialiased selection:bg-primary-container selection:text-white">
      ...
      <main>
        ...
        {/* Standardized Kids Product Grid */}
          ...
          {isLoading ? (
            ...
            <div className="py-24 text-center space-y-3">
              ...
              <p className="font-body-md text-secondary text-sm">
                No matching petite garments are currently available.
              </p>
            </div>
          ) : (
