...
interface SaleHeroBannerProps {
  banners?: CategorySlide[];
  autoPlayInterval?: number;
}

export const SaleHeroBanner: React.FC<SaleHeroBannerProps> = ({
  banners = categoryBannersData.sale || [],
  ...
  return (
    <section
      className="w-full relative h-[50vh] min-h-[420px] max-h-[480px] overflow-hidden select-none bg-surface-container"
    ...
    >
      ...
      {/* Hero Content with Dynamic Slide Data */}
      <div className="relative z-20 h-full max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex flex-col justify-center">
        <div className="max-w-2xl text-white">
          <span className="inline-block font-label-caps text-[11px] uppercase tracking-[0.3em] mb-3 text-white/80">
            {currentBanner.badge || 'Private Archive // Limited Access'}
          </span>
          <h1 className="font-headline-lg text-[44px] md:text-[60px] leading-tight mb-3 text-white drop-shadow-md">
            {currentBanner.title || 'Private Sale'}
          </h1>
          <p className="font-body-lg text-white/90 max-w-lg text-sm md:text-base leading-relaxed">
            {currentBanner.subtitle || 'Curated archive selections and architectural essentials at privileged private pricing.'}
          </p>
        </div>
      </div>
